...
function DataForm(props) {
  ...
  async function getRateMean(filePaths) {
    ...
    await Promise.all(taskList).then((rateList) => {
      ...
      if (rateList.length > 0) {
        ...
        setCV(cv)
        let concentration =
          (params['gradient'] * rateMean + params['intercept']) * dilutionRatio
  ...
  function handleInputConcChange(e) {
    ...
    setRelativeDeviation(
      divide(concentration - currentInputConc, currentInputConc)
    )
    let dataRow = {
      [index]: [parseFloat(currentInputConc), parseFloat(concentration)],
    }
